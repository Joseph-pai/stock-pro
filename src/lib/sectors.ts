/**
 * Taiwan Stock Market Sectors & Market Types
 */

export type MarketType = 'TWSE' | 'TPEX';

export const MARKET_NAMES: Record<MarketType, string> = {
    TWSE: '上市 (TWSE)',
    TPEX: '上櫃 (TPEX)'
};

/**
 * Universal Industry Code Map
 * Ensures badges are always translated even if the ID varies between APIs
 */
export const INDUSTRY_MAP: Record<string, string> = {
    '01': '水泥工業',
    '02': '食品工業',
    '03': '塑膠工業',
    '04': '紡織纖維',
    '05': '電機機械',
    '06': '電器電纜',
    '07': '化學工業',
    '21': '化學工業',
    '08': '玻璃陶瓷',
    '09': '造紙工業',
    '10': '鋼鐵工業',
    '11': '橡膠工業',
    '12': '汽車工業',
    '13': '電子全部',
    '14': '建材營造',
    '15': '航運業',
    '16': '觀光餐旅',
    '17': '金融保險',
    '18': '貿易百貨',
    '19': '綜合',
    '20': '其他',
    '22': '生技醫療',
    '23': '油電燃氣',
    '24': '半導體業',
    '25': '電腦及週邊',
    '26': '光電業',
    '27': '通信網路',
    '28': '電子零組件',
    '29': '電子通路',
    '30': '資訊服務',
    '31': '其他電子',
    '32': '綠能環保',
    '33': '數位雲端',
    '34': '運動休閒',
    '35': '居家生活',
    '36': '觀光餐旅',
    '37': '其他',
    '38': '文化創意',
    '39': '電子商務',
    '40': '農業科技',
    // Search string variations from TPEX API
    '電腦及週邊業': '電腦週邊',
    '電腦及週邊設備業': '電腦週邊',
    '電腦及週邊設備': '電腦週邊',
    '光電': '光電業',
    '生技醫療業': '生技醫療',
    '電子零組件業': '電子零組件',
    '通信網路業': '通信網路',
    '半導體': '半導體業',
    '半導體業': '半導體業',
    '電機機械': '電機機械',
    '電機機械業': '電機機械',
    '油電燃氣業': '油電燃氣',
    '文化創意業': '文化創意',
    '農業科技業': '農業科技',
    '觀光餐旅業': '觀光餐旅',
};

// Selection Sectors Mapping (Targeted for official MI_INDEX & TPEX code queries)
// IDs match the 'type' parameter for TWSE and 'se' parameter for TPEX
export const SECTORS = {
    TWSE: [
        { id: 'ALL', name: '全部類股' },
        { id: '01', name: '水泥工業' },
        { id: '02', name: '食品工業' },
        { id: '03', name: '塑膠工業' },
        { id: '04', name: '紡織纖維' },
        { id: '05', name: '電機機械' },
        { id: '06', name: '電器電纜' },
        { id: '07', name: '化學工業' },
        { id: '08', name: '玻璃陶瓷' },
        { id: '09', name: '造紙工業' },
        { id: '10', name: '鋼鐵工業' },
        { id: '11', name: '橡膠工業' },
        { id: '12', name: '汽車工業' },
        { id: '24', name: '半導體業' },
        { id: '25', name: '電腦及週邊' },
        { id: '26', name: '光電業' },
        { id: '27', name: '通信網路' },
        { id: '28', name: '電子零組件' },
        { id: '29', name: '電子通路' },
        { id: '30', name: '資訊服務' },
        { id: '31', name: '其他電子' },
        { id: '14', name: '建材營造' },
        { id: '15', name: '航運業' },
        { id: '36', name: '觀光餐旅' },
        { id: '22', name: '生技醫療' },
        { id: '17', name: '金融保險' },
        { id: '18', name: '貿易百貨' },
        { id: '23', name: '油電燃氣' },
        { id: '32', name: '綠能環保' },
        { id: '33', name: '數位雲端' },
        { id: '34', name: '運動休閒' },
        { id: '35', name: '居家生活' },
        { id: '20', name: '其他' },
    ],
    TPEX: [
        { id: 'AL', name: '全部類股' },
        { id: '02', name: '食品工業' },
        { id: '03', name: '塑膠工業' },
        { id: '04', name: '紡織纖維' },
        { id: '05', name: '電機機械' },
        { id: '06', name: '電器電纜' },
        { id: '21', name: '化學工業' },
        { id: '08', name: '玻璃陶瓷' },
        { id: '10', name: '鋼鐵工業' },
        { id: '11', name: '橡膠工業' },
        { id: '12', name: '汽車工業' },
        { id: '22', name: '生技醫療' },
        { id: '24', name: '半導體業' },
        { id: '25', name: '電腦及週邊' },
        { id: '26', name: '光電業' },
        { id: '27', name: '通信網路' },
        { id: '28', name: '電子零組件' },
        { id: '29', name: '電子通路' },
        { id: '30', name: '資訊服務' },
        { id: '31', name: '其他電子' },
        { id: '14', name: '建材營造' },
        { id: '15', name: '航運業' },
        { id: '36', name: '觀光餐旅' },
        { id: '17', name: '金融業' },
        { id: '18', name: '貿易百貨' },
        { id: '23', name: '油電燃氣' },
        { id: '32', name: '綠能環保' },
        { id: '33', name: '數位雲端' },
        { id: '34', name: '運動休閒' },
        { id: '35', name: '居家生活' },
        { id: '38', name: '文化創意' },
        { id: '40', name: '農業科技' },
        { id: '20', name: '其他' },
    ]
};
